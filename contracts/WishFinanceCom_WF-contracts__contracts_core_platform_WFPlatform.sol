 ...
 * @title WF Platform.
 *
 * The official WF assets platform powering WF token, and possibly
 ...
 * didn't happen yet.
 */
contract WFPlatform is Object, WFPlatformEmitter {

    using SafeMath for uint;

    uint constant WF_PLATFORM_SCOPE = 15000;
    uint constant WF_PLATFORM_PROXY_ALREADY_EXISTS = WF_PLATFORM_SCOPE + 0;
    uint constant WF_PLATFORM_CANNOT_APPLY_TO_ONESELF = WF_PLATFORM_SCOPE + 1;
    uint constant WF_PLATFORM_INVALID_VALUE = WF_PLATFORM_SCOPE + 2;
    uint constant WF_PLATFORM_INSUFFICIENT_BALANCE = WF_PLATFORM_SCOPE + 3;
    uint constant WF_PLATFORM_NOT_ENOUGH_ALLOWANCE = WF_PLATFORM_SCOPE + 4;
    uint constant WF_PLATFORM_ASSET_ALREADY_ISSUED = WF_PLATFORM_SCOPE + 5;
    uint constant WF_PLATFORM_CANNOT_ISSUE_FIXED_ASSET_WITH_INVALID_VALUE = WF_PLATFORM_SCOPE + 6;
    uint constant WF_PLATFORM_CANNOT_REISSUE_FIXED_ASSET = WF_PLATFORM_SCOPE + 7;
    uint constant WF_PLATFORM_SUPPLY_OVERFLOW = WF_PLATFORM_SCOPE + 8;
    uint constant WF_PLATFORM_NOT_ENOUGH_TOKENS = WF_PLATFORM_SCOPE + 9;
    uint constant WF_PLATFORM_INVALID_NEW_OWNER = WF_PLATFORM_SCOPE + 10;
    uint constant WF_PLATFORM_ALREADY_TRUSTED = WF_PLATFORM_SCOPE + 11;
    uint constant WF_PLATFORM_SHOULD_RECOVER_TO_NEW_ADDRESS = WF_PLATFORM_SCOPE + 12;
    uint constant WF_PLATFORM_ASSET_IS_NOT_ISSUED = WF_PLATFORM_SCOPE + 13;
    uint constant WF_PLATFORM_ACCESS_DENIED_ONLY_OWNER = WF_PLATFORM_SCOPE + 14;
    uint constant WF_PLATFORM_ACCESS_DENIED_ONLY_PROXY = WF_PLATFORM_SCOPE + 15;
    uint constant WF_PLATFORM_ACCESS_DENIED_ONLY_TRUSTED = WF_PLATFORM_SCOPE + 16;
    uint constant WF_PLATFORM_INVALID_INVOCATION = WF_PLATFORM_SCOPE + 17;
    uint constant WF_PLATFORM_HOLDER_EXISTS = WF_PLATFORM_SCOPE + 18;
    ...
    function _error(uint _errorCode, bytes32 _message) internal returns(uint) {
        WFPlatformEmitter(eventsHistory).emitError(_message);
        return _errorCode;
    }
    ...
    function setupEventsHistory(address _eventsHistory) returns(uint errorCode) {
        ...
        if (eventsHistory != 0x0 && eventsHistory != _eventsHistory) {
            return WF_PLATFORM_INVALID_INVOCATION;
        }
    ...
    function checkIsOnlyOwner(bytes32 _symbol) internal constant returns(uint errorCode) {
        ...
        return _error(WF_PLATFORM_ACCESS_DENIED_ONLY_OWNER, "Only owner: access denied");
    }
    ...
    function checkIsOnlyProxy(bytes32 _symbol) internal constant returns(uint errorCode) {
        ...
        return _error(WF_PLATFORM_ACCESS_DENIED_ONLY_PROXY, "Only proxy: access denied");
    }
    ...
    function shouldBeTrusted(address _from, address _to) internal constant returns(uint errorCode) {
        ...
        return _error(WF_PLATFORM_ACCESS_DENIED_ONLY_TRUSTED, "Only trusted: access denied");
    }
    ...
    function setProxy(address _address, bytes32 _symbol) returns(uint errorCode) {
        ...
        if (proxies[_symbol] != 0x0) {
            return WF_PLATFORM_PROXY_ALREADY_EXISTS;
        }
    ...
    returns (uint errorCode, uint count)
    {
        ...
        for(uint idx = 0; idx < addresses.length && msg.gas > 110000; idx++) {
            ...
            if (value == 0) {
                _error(WF_PLATFORM_INVALID_VALUE, "Cannot send 0 value");
                continue;
            }

            if (getHolderId(addresses[idx]) > 0) {
                _error(WF_PLATFORM_HOLDER_EXISTS, "Already transfered");
                continue;
            }

            if (_balanceOf(senderId, _symbol) < value) {
                _error(WF_PLATFORM_INSUFFICIENT_BALANCE, "Insufficient balance");
                continue;
            }

            if (msg.sender == addresses[idx]) {
                _error(WF_PLATFORM_CANNOT_APPLY_TO_ONESELF, "Cannot send to oneself");
                continue;
            }
            ...
            WFPlatformEmitter(eventsHistory).emitTransfer(msg.sender, addresses[idx], _symbol, value, "");
    ...
    function _transfer(uint _fromId, uint _toId, uint _value, bytes32 _symbol, string _reference, uint _senderId) internal returns(uint) {
        ...
        if (_fromId == _toId) {
            return _error(WF_PLATFORM_CANNOT_APPLY_TO_ONESELF, "Cannot send to oneself");
        }
        // Should have positive value.
        if (_value == 0) {
            return _error(WF_PLATFORM_INVALID_VALUE, "Cannot send 0 value");
        }
        // Should have enough balance.
        if (_balanceOf(_fromId, _symbol) < _value) {
            return _error(WF_PLATFORM_INSUFFICIENT_BALANCE, "Insufficient balance");
        }
        // Should have enough allowance.
        if (_fromId != _senderId && _allowance(_fromId, _senderId, _symbol) < _value) {
            return _error(WF_PLATFORM_NOT_ENOUGH_ALLOWANCE, "Not enough allowance");
        }
        ...
        WFPlatformEmitter(eventsHistory).emitTransfer(_address(_fromId), _address(_toId), _symbol, _value, _reference);
    ...
    function issueAsset(bytes32 _symbol, uint _value, string _name, string _description, uint8 _baseUnit, bool _isReissuable) returns(uint errorCode) {
        ...
        if (_value == 0 && !_isReissuable) {
            return _error(WF_PLATFORM_CANNOT_ISSUE_FIXED_ASSET_WITH_INVALID_VALUE, "Cannot issue 0 value fixed asset");
        }
        // Should not be issued yet.
        if (isCreated(_symbol)) {
            return _error(WF_PLATFORM_ASSET_ALREADY_ISSUED, "Asset already issued");
        }
        ...
        // Call Stack Depth Limit reached: n/a after HF 4;
        // Recursive Call: safe, all changes already made.
        WFPlatformEmitter(eventsHistory).emitIssue(_symbol, _value, _address(holderId));
        return OK;
    }
    ...
    function reissueAsset(bytes32 _symbol, uint _value) returns(uint errorCode) {
        ...
        // Should have positive value.
        if (_value == 0) {
            return _error(WF_PLATFORM_INVALID_VALUE, "Cannot reissue 0 value");
        }
        Asset asset = assets[_symbol];
        // Should have dynamic supply.
        if (!asset.isReissuable) {
            return _error(WF_PLATFORM_CANNOT_REISSUE_FIXED_ASSET, "Cannot reissue fixed asset");
        }
        // Resulting total supply should not overflow.
        if (asset.totalSupply + _value < asset.totalSupply) {
            return _error(WF_PLATFORM_SUPPLY_OVERFLOW, "Total supply overflow");
        }
    ...
    function revokeAsset(bytes32 _symbol, uint _value) returns(uint) {
        // Should have positive value.
        if (_value == 0) {
            return _error(WF_PLATFORM_INVALID_VALUE, "Cannot revoke 0 value");
        }
        Asset asset = assets[_symbol];
        uint holderId = getHolderId(msg.sender);
        // Should have enough tokens.
        if (asset.wallets[holderId].balance < _value) {
            return _error(WF_PLATFORM_NOT_ENOUGH_TOKENS, "Not enough tokens to revoke");
        }
        ...
        // Call Stack Depth Limit reached: n/a after HF 4;
        // Recursive Call: safe, all changes already made.
        WFPlatformEmitter(eventsHistory).emitRevoke(_symbol, _value, _address(holderId));
    ...
    function changeOwnership(bytes32 _symbol, address _newOwner) returns(uint errorCode) {
        ...
        if (_newOwner == 0x0) {
            return _error(WF_PLATFORM_INVALID_NEW_OWNER, "Can't change ownership to 0x0");
        }
        ...
        if (asset.owner == newOwnerId) {
            return _error(WF_PLATFORM_CANNOT_APPLY_TO_ONESELF, "Cannot pass ownership to oneself");
        }
        ...
        // Call Stack Depth Limit reached: n/a after HF 4;
        // Recursive Call: safe, all changes already made.
        WFPlatformEmitter(eventsHistory).emitOwnershipChange(oldOwner, _address(newOwnerId), _symbol);
        return OK;
    }
    ...
    function trust(address _to) returns(uint) {
        ...
        if (fromId == getHolderId(_to)) {
            return _error(WF_PLATFORM_CANNOT_APPLY_TO_ONESELF, "Cannot trust to oneself");
        }
        // Should trust to yet untrusted.
        if (isTrusted(msg.sender, _to)) {
            return _error(WF_PLATFORM_ALREADY_TRUSTED, "Already trusted");
        }
    ...
    function recover(address _from, address _to) returns(uint errorCode) {
        ...
        if (getHolderId(_to) != 0) {
            return _error(WF_PLATFORM_SHOULD_RECOVER_TO_NEW_ADDRESS, "Should recover to new address");
        }
        ...
        // Recursive Call: safe, all changes already made.
        WFPlatformEmitter(eventsHistory).emitRecovery(from, _to, msg.sender);
        return OK;
    }
    ...
    function _approve(uint _spenderId, uint _value, bytes32 _symbol, uint _senderId) internal returns(uint) {
        // Asset should exist.
        if (!isCreated(_symbol)) {
            return _error(WF_PLATFORM_ASSET_IS_NOT_ISSUED, "Asset is not issued");
        }
        // Should allow to another holder.
        if (_senderId == _spenderId) {
            return _error(WF_PLATFORM_CANNOT_APPLY_TO_ONESELF, "Cannot approve to oneself");
        }
        assets[_symbol].wallets[_senderId].allowance[_spenderId] = _value;
        // Internal Out Of Gas/Throw: revert this transaction too;
        // Call Stack Depth Limit reached: revert this transaction too;
        // Recursive Call: safe, all changes already made.
        WFPlatformEmitter(eventsHistory).emitApprove(_address(_senderId), _address(_spenderId), _symbol, _value);
        if (proxies[_symbol] != 0x0) {
            // Internal Out Of Gas/Throw: revert this transaction too;
            // Call Stack Depth Limit reached: n/a after HF 4;
            // Recursive Call: safe, all changes already made.
