...
/// @title BenkiICO contract - Takes funds from users and issues tokens.
...
contract BenkiICO {
     ...
     * External contracts
     */
    BenkiToken public benkiToken = BenkiToken(0x0);
        ...
        returns (uint)
    {
        ...
        if (!benkiToken.issueTokens(msg.sender, tokenCount)) {
        ...
        returns (uint)
    {
        ...
        if (!benkiToken.issueTokens(beneficiary, _tokenCount)) {
            // Tokens could not be issued.
            throw;
        }
        return _tokenCount;
        ...
        returns (bool)
    {
        if (isICOActive == true) {
            ...
            if (!benkiToken.issueTokens(multisig, founderBonus)) {
                // Tokens could not be issued.
                throw;
            }
        }
    ...
    function BenkiICO(address _multisig) {
        // Set founder address
        founder = msg.sender;
