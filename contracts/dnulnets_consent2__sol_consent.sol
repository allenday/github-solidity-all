...
/*
 * This is the contract that holds a companys URL for getting a user consent file
 * based on his userid and password at that company.
 *
 */
contract CompanyURL {

  address private owner;        /* The owner of the contract */
  string private companyName;   /* The name of the company */
  string private url;           /* The URL to the HTTP REST call */


  /*
   * The name of the company and the URL to the REST service is "hardcoded" into the contract upon
   * creation.
   *
   */
  function CompanyURL (string _companyName, string _url) public
  {
    owner = tx.origin;
  ...
  function getURL () constant public returns (string)
  {
    return url;
  }
...
/*
 * This is the contract that holds all companies URL:s for getting a users consent
 * file.
 *
 * It is a directory of companies that support automatic consent file retrieval.
 */
contract CompanyDirectory {

  address private   owner;           /* The owner of the contract */
  address[] private listOfCompanies; /* The list of all companies CompanyURL contracts */

  /*
   * Creates the company directory, there should only be one of theese around. The contract hash for
   * this initial contract is the default contract used by teh applications.
   *
   */
  function ConsentDirectory () public
  {
    owner = tx.origin;
  }
  ...
  /*
   * Adds a companyURL to the list of companies that support the URL REST call
   *
   */
  function addCompanyURL (string _companyName, string _url) onlyBy (owner) public
  {
    CompanyURL curl = new CompanyURL (_companyName, _url);
    listOfCompanies.push (curl);
  }
