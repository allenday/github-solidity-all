...
contract Association is owned {
    ...
    function dividend() {
      uint quote = msg.value / sharesTokenAddress.totalSupply();
      if(quote==0) throw;
      uint totalOwners = sharesTokenAddress.totalOwners();

      for(var i=0 ; i< totalOwners ; i++) {
        address owner = sharesTokenAddress.owners(i);
        uint balance = sharesTokenAddress.balanceOf(owner);
        if(balance>0) {
          owner.send( quote * balance );
        }
      }
    }
