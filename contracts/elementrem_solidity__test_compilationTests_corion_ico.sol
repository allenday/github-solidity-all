...
contract ico is safeMath {
    ...
    function setICOElePrice(uint256 value) external {
        /*
    ...
    function closeICO() external {
        /*
            ...
            Elements which are situated in this contract will be sent to the address of the fundation.
        */
        require( msg.sender == owner );
        require( block.number > icoDelay );
        require( ! closed );
    ...
    function receiveFunds() external {
        ...
        */
        require( aborted );
        require( brought[msg.sender].ele > 0 );
        uint256 _val = brought[msg.sender].ele * 90 / 100;
    ...
    function buy(address beneficiaryAddress, address affilateAddress) payable returns (bool success) {
        /*
            ...
            If there is not at least 0.2 element balance on the beneficiaryAddress then the amount of the element which was intended for the purchase will be reduced by 0.2 and that will be sent to the address of the beneficiary.
        ...
        */
        ...
        if ( beneficiaryAddress.balance < 0.2 element ) {
            require( beneficiaryAddress.send(0.2 element) );
            _value = safeSub(_value, 0.2 element);
        }
        var _reward = getIcoReward(_value);
        require( _reward > 0 );
        require( token(tokenAddr).mint(beneficiaryAddress, _reward) );
        brought[beneficiaryAddress].ele = safeAdd(brought[beneficiaryAddress].ele, _value);
        ...
        uint256 extra;
        if ( affilateAddress != 0x00 && ( brought[affilateAddress].ele > 0 || interestDB[affilateAddress][0].amount > 0 ) ) {
    ...
    function getIcoReward(uint256 value) public constant returns (uint256 reward) {
        /*
            ...
            @value The amount of element for the purchase
        ...
        */
        reward = (value * 1e6 * icoExchangeRate / icoExchangeRateM / 1 element) * (ICObonus() + 100) / 100;
